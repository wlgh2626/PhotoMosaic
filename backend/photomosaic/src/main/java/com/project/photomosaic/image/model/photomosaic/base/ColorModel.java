...
/**
 * CURRNENTLY UNUSED. Converts between different color models. Mainly between
 * RGB and YUV Only supports 8 bit color depth
 */
public class ColorModel {
	public static final Double[][] RGB_YUV_MATRIX = { { 0.299, 0.587, 0.114 }, { -0.14713, -0.28887, 0.436 },
			{ 0.615, -0.51499, -0.10001 } };

	public static final Double[][] YUV_RGB_MATRIX = { { 1.0, 0.0, 1.13983 }, { 1.0, -0.39465, -0.58060 },
			{ 1.0, 2.03211, 0.0 } };
	...
	public static int RGBToYUV(int r, int g, int b) {
		...
		for (int i = 0; i < 3; i++) {
			...
			if (yuv[i] > 255)
				throw new RuntimeException("YUV exceeds 8-bit depth.");
		}
		return yuv[0].intValue() << 16 + yuv[1].intValue() << 8 + yuv[2].intValue(); // returns YUV value with 8 bit
																						// depth each
	}
	...
	public static int YUVToRGB(int y, int u, int v) {
		...
		for (int i = 0; i < 3; i++) {
			...
			if (rgb[i] > 255)
				throw new RuntimeException("RGB exceeds 8-bit depth.");
		}
