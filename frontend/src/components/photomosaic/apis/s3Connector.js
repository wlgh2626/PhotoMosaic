...
const s3ProviderUrl =
  process.env.REACT_APP_S3_CONNECTOR_URL +
  ":" +
  process.env.REACT_APP_S3_CONNECTOR_PORT;

export default async function S3Connector(
  directory,
  originalImage,
  sampleList
) {
  async function upload(image, imageKey) {
    ...
    const url = await fetch(
      s3ProviderUrl + "/requestS3Url?key=" + imageKey
    ).then((res) => {
      return res.text();
    });
  ...
  if (originalImage && sampleList.length !== 0) {
    ...
    const originalUploaded = await upload(
      originalImage,
      directory + "/original" + Path.extname(originalImage.name)
    );
    const samplesUploaded = await uploadMultiple(
      sampleList,
      directory + "/sample"
    );
